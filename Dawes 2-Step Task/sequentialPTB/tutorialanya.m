...
Screen('DrawTexture',w,s(4,1).norm,[],leftposvect);
Screen('DrawTexture',w,s(4,2).norm,[],rightposvect);
DrawFormattedText(w,'Each of these boxes has a certain chance of containing one dollar.  The aim is to find a box with a higher chance of winning money, and choose it.'...
    ,'center',ytext,[],wrap);
Screen('Flip',w,[]);
KbWait([],2);
Screen('DrawTexture',w,s(4,1).norm,[],leftposvect);
Screen('DrawTexture',w,s(4,2).norm,[],rightposvect);
DrawFormattedText(w,'You select the left box by pressing U, and the right box by pressing I.',...
    'center',ytext,[],wrap);
Screen('Flip',w);
KbWait([],2);
...
%Screen('DrawTexture',w,s(4,1).norm,[],leftposvect);
%Screen('DrawTexture',w,s(4,2).norm,[],rightposvect);
%DrawFormattedText(w, 'In this demonstration you wont be playing for real money. In the actual experiment, you will be.','center',ytext,[],wrap);
%Screen('Flip',w);
%KbWait([],2);
...
DrawFormattedText(w,['To get a feel for this, have a practice:  Every time you select the box is like playing a game of chance, with a 60% of winning money.'...
    '\n\n' 'In this demonstration you wont be playing for real money. In the actual experiment, you will be.' '\n\n' 'Press the U key to select the box and try this out.' '\n\n'  'You will get 10 tries.'],'center',ytext,[],wrap);
Screen('Flip',w);
...
% screen 16
DrawFormattedText(w,'Because the chances of win for each box can slowly change, a box that starts out better, can turn worse later, or a worse one can turn better.  So finding the box that is best right now requires continual concentration.','center',ytext,[],wrap);
Screen('Flip',w);
KbWait([],2);
...
DrawFormattedText(w,['There is also another pair of boxes.  In the game, you start each trial by choosing between these two.' '\n\n'...
...
DrawFormattedText(w,['You found a dollar.' '\n\n' 'Press any key to continue.'],'center',ytext,[],wrap);
Screen('Flip',w);
KbWait([],2);
...
Screen('DrawTexture',w,s(1,1).norm,[],leftposvect);
Screen('DrawTexture',w,s(1,2).norm,[],rightposvect);
DrawFormattedText(w,['This may all sound complicated, so lets review:' '\n\n' 'Each trial consists of two stages. On the first stage you are choosing between two boxes (in the tutorial,the orange ones), each of them has certain chance of leading you to either pair of second stage boxes (in the tutorial, the purple or turquoise ones).']...
    ,'center',ytext,[],wrap);
Screen('Flip',w);
KbWait([],2);
...
Screen('DrawTexture',w,s(3,1).norm,[],leftposvect - [0 120 0 120]);
Screen('DrawTexture',w,s(3,2).norm,[],rightposvect - [0 120 0 120]);
DrawFormattedText(w,['All second stage boxes have different chances of winning money which change over the course of the game.'],...
    'center',ytext,[],wrap);
Screen('Flip',w);
KbWait([],2)
...
DrawFormattedText(w,['Hints:' '\n\n' 'The game is hard, so you will need to concentrate, but dont be afraid to trust your instincts.' '\n\n'...
    ...
    '\n\n' 'This is how the game involves skill as well as luck:  finding which boxes are currently best and following when they change.'],'center','center',[],wrap);
Screen('Flip',w);
KbWait([],2);
...
DrawFormattedText(w,['Hints:' '\n\n' 'Remember, none of the boxes know what the others are doing.  Just because one box is poor does not mean any others have to be rich.' '\n\n' ...
    ...
    'The computer is not trying to catch you out or trick you.  Remember, it is a game of both skill and chance, so good luck!'],'center','center',[],wrap);
Screen('Flip',w)
KbWait([],2)

DrawFormattedText(w,['Finally:' '\n\n' 'The way we will determine the amount of money you win is by keeping track of whether you win a dollar on each trial.  At the end the computer will randomly pick one-third of the trials and we will pay you the total money you won on all of those trials.' '\n\n'...
   'For instance, you might get paid what you earn on trials 1,2,5,7, etc.  So you should play every trial as though there is one dollar at stake, because you dont know whether that trial will be one of the ones you will be paid for!'],'center','center',[],wrap);
Screen('Flip',w);
KbWait([],2);


DrawFormattedText(w,[ 'If you have any questions about the task, you need to ask the experimenter now.' '\n\n' 'Press any button to start the main task.'],...
    'center',ytext,[],wrap);
Screen('Flip',w);
KbWait([],2);
