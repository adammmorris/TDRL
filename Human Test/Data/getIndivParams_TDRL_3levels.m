...
function [optimalParams] = getIndivParams_TDRL_3levels(model,type,id,a1,s2,a2,s3,a3,re,round,normed,starts,A,b,bounds,tosslist)
...
for thisStart = 1:numStarts
    ...
    parfor thisSubj = 1:numSubjects
        ...
        if ~any(tosslist == thisSubj)
            ...
            % Do patternsearch
            [max_params(:,thisStart,thisSubj),lik(thisStart,thisSubj),~] = patternsearch(@(params) model(type,params,a1(index),s2(index),a2(index),s3(index),a3(index),re(index),round(index),normed),starts(thisStart,:),A,b,[],[],bounds(1,:),bounds(2,:),options);
        end
    end
end
