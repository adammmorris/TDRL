...
function [likelihood] = getIndivLike_AC_comb(type, x, A1, S2, A2, Re, round, normed)
...
for thisRound = 1:length(A1)
    ...
    if round(thisRound) > practiceCutoff
        ...
        if reward >= 0
            ...
            delta = gammaP .* values(state2) - values(1);
            policy(1,action1) = policy(1,action1) + betaP * delta;
            values(1) = values(1) + alphaP * delta;
