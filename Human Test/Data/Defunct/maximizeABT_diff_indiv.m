% This is going to maximize the two learning rates plus temperature, all
% individually
function [results] = maximizeABT_diff_indiv(id, A1, S2, A2, Re, tosslist, start, separateTemp)
...
for thisSubject = 1:numSubjects
    ...
    if sum(tosslist == thisSubject) == 0
        ...
        if separateTemp == 0
            [max_params, lik, ~] = patternsearch(@(params) getLikelihood_diff(params,A1(thisIndex), S2(thisIndex), A2(thisIndex), Re(thisIndex), separateTemp), start,[],[],[],[],[0 0 0 0 .1],[1 1 1 1 2],options);
        elseif separateTemp == 1
            [max_params, lik, ~] = patternsearch(@(params) getLikelihood_diff(params,A1(thisIndex), S2(thisIndex), A2(thisIndex), Re(thisIndex), separateTemp), start,[],[],[],[],[0 0 0 0],[1 1 2 2],options);
        end
