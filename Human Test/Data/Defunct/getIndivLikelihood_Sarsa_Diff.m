%% getIndivLikelihood_Sarsa_Diff
% This is a normal SARSA model w/ eligibility traces (differentiated
%   alphas)

%% Params
% x should be [alpha1_R alpha1_P alpha2_R alpha2_P elig temp]

function [likelihood] = getIndivLikelihood_Sarsa_Diff(x, A1, S2, A2, Re)
...
for thisRound = 1:length(A1)
    ...
    if reward >= 0
        ...
        Q(1,action1) = Q(1,action1) + alpha1_R * delta;

        % 2nd level
        delta = reward - Q(state2,action2);
        Q(state2,action2) = Q(state2,action2) + alpha2_R * delta;
        Q(1,action1) = Q(1,action1) + elig * alpha1_R * delta;
    else
        % 1st level
        delta = gamma*Q(state2,action2) - Q(1,action1);
        Q(1,action1) = Q(1,action1) + alpha1_P * delta;

        % 2nd level
        delta = reward - Q(state2,action2);
        Q(state2,action2) = Q(state2,action2) + alpha2_P * delta;
        Q(1,action1) = Q(1,action1) + elig * alpha1_P * delta;
    end
end
