%% getIndivLikelihood_Sarsa_DiffElig
% This is a normal SARSA model w/ eligibility traces (differentiated
%   alphas)

%% Params
% x should be [alpha1 alpha2 eligR eligP temp]

function [likelihood] = getIndivLikelihood_Sarsa_DiffElig(x, A1, S2, A2, Re)
...
for thisRound = 1:length(A1)
    ...
    if reward >= 0
        ...
        Q(1,action1) = Q(1,action1) + eligR * alpha1 * delta;
    else
        % 1st level
        delta = gamma*Q(state2,action2) - Q(1,action1);
        Q(1,action1) = Q(1,action1) + alpha1 * delta;
        ...
        delta = reward - Q(state2,action2);
        Q(state2,action2) = Q(state2,action2) + alpha2 * delta;
        Q(1,action1) = Q(1,action1) + eligP * alpha1 * delta;
    end
end
