...
var curShape;
var curReward_disp;
var curState = {shape:'', texture:'', last_symbol_shape:'', last_symbol_texture:'', level:''};
var curStateStart;
...
var level1_symbols = [{shape:"square", texture:"lined"}, {shape:"square", texture:"empty"}];
var level2_symbols = [{shape:"star", texture:"black"}, {shape:"diamond", texture:"black"}, {shape:"pentagon", texture:"black"}, {shape:"octagon", texture:"black"}];
...
function generateSymbolSequence() {
	...
	*/
	var numMoves = 2; // number of decision points
	var numActions = 2; // number of choices per decision point
	var numPlays = 50; // number of plays each participant gets
	var cycles = 16;
	var blocks = 4;
...
function stateTransition(oldState, action) {
	...
	*/
	...
	else if (oldState.level == 2) {
		newState.level = 1;
	}
	else {
		alert("An error has occured!  Error code: lkj30sl");
	}
