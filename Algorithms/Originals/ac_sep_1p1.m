...
for thisBoard = 1:numBoards
    ...
    for thisPlay = 1:numPlays
        ...
        policyR(state,action) = policyR(state,action) + betaR.*(0 - valuesR(state));
        policyP(state,action) = policyP(state,action) + betaP.*(0 - valuesP(state));


        % Update critic

        valuesR(state) = valuesR(state) + alphaR.*(0 - valuesR(state));
        valuesP(state) = valuesP(state) + alphaP.*(0 - valuesP(state));
