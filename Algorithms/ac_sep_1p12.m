...
function [totEarnings, stdEarnings, learnCurve, likelihood, likelihood_alt, slips] = ac_sep_1p12(x, numInstances, numPlays, boardPath, x_alt)
...
% in 1p12, this is changed to be static, not dynamic percentages
...
numPlaysBeforeShift = 5; % If you don't want LT stochasticism, set this to numPlays
avg_shift_lt = 0;
std_shift_lt = 7.5;
stayInRange = 1;
...
std_shift_st = 1; % If you don't want ST stochasticism, set this to 0
        ...
        for thisMove = 1:numMoves
            ...
            if stochastic_counter > numPlaysBeforeShift
                ...
                for thisState = 1 : numStates
                    % For now, ignore any states w/ 0 reward
                    if boards(thisState, thisBoard, currentSwitch) ~= 0
                        % Calculate shift
                        shift = round(avg_shift_lt + std_shift_lt .* randn(1,1));

                        % Shift
                        % Do we want to implement stay in range?
                        if stayInRange == 1
                            if (boards(thisState, thisBoard, currentSwitch) + shift) > rewardRange || (boards(thisState, thisBoard, currentSwitch) + shift) < (-rewardRange)
                                boards(thisState, thisBoard, currentSwitch) = boards(thisState, thisBoard, currentSwitch) - shift;
                            else
                                boards(thisState, thisBoard, currentSwitch) = boards(thisState, thisBoard, currentSwitch) + shift;
                            end
            ...
            if std_shift_st > 0 && reward ~= 0
                shift = round(std_shift_st .* randn(1));
                reward = reward + shift;
            end
