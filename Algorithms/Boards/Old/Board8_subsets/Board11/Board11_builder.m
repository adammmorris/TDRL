...
% Board11 is single-tier, punishments are NOT doubled, more punishments than
%   rewards - w/ multiple branches of all punishments
% (This is Board8 with more punishments than rewards)
...
for i = 1:numBoards
    if randomized == 1
        ...
        % First 4 trees
        for k = 1:4
            trees(k, 1:8) = randsample(-25:-1, 8, true);
        end
        
        % Next 2 trees
        for k = 5:6
            trees(k, 1:4) = randsample(1:25, 4, true);
            trees(k, 5:8) = randsample(-25:-1, 4, true);
...
save(strcat(currentPath, '/Board11_soft.mat'), 'boards', 'transitions', 'numStates', 'numMoves', 'numActions', 'numBoards')