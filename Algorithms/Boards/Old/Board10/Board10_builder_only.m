...
for i = 1:numBoards
    ...
    elseif randomized == 2
        ...
        % on level 3, half punishments, half zeros ('only punishments'
        %   condition)
        
        % level 2 - all zeros here
        %boards(2:5, i) = randsample(-15:15, numActions, true);
        
        % level 3 - 6:21
        boards(6:13, i) = randsample(-45:-15, 8, true);
        boards(6:21, i) = randsample(boards(6:21, i), length(boards(6:21, i))); % Permute
        
        boards(22:85, i) = randsample(-60:60, numActions ^ 3, true);
    else
        % no randomization
...
save(strcat(currentPath, '/Board10_only.mat'), 'boards', 'transitions', 'numStates', 'numMoves', 'numActions', 'numBoards')