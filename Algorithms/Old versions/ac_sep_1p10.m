...
function [totEarnings, stdEarnings, learnCurve, likelihood, likelihood_alt] = ac_sep_1p10(x, x_alt, numInstances, numPlays, boardPath)
...
numPlaysBeforeShift = 10; % If you don't want LT stochasticism, set this to numPlays
avg_shift_lt = 30;
std_shift_lt = 15;
...
%   by -1
std_shift_st = 10; % If you don't want ST stochasticism, set this to 0
    ...
    for thisPlay = 1:numPlays
        ...
        for thisMove = 1:numMoves
            ...
            probs = probs_numerator ./ probs_denominator;
                ...
                probs_numerator_alt = exp(tempR .* policyR_alt(state,:) + tempP .* policyP_alt(state,:));
            ...
            likelihood_alt = likelihood_alt + log(probs_alt(action));
            ...
            policyP(state,action) = policyP(state,action) + betaP.*(((min(reward,0) + gammaP.*valuesP(newstate)) - valuesP(state)));
                ...
                policyR_alt(state,action) = policyR_alt(state,action) + betaR.*(((max(reward,0) + gammaR.*valuesR_alt(newstate)) - valuesR_alt(state)));
                policyP_alt(state,action) = policyP_alt(state,action) + betaP.*(((min(reward,0) + gammaP.*valuesP_alt(newstate)) - valuesP_alt(state)));
